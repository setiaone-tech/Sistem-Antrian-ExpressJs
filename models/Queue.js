...
const QueueSchema = new mongoose.Schema({
    ...
    verification_code: { type: String, required: true }, // Tambahkan ini
    status: { type: String, enum: ['waiting', 'Telah Diproses'], default: 'waiting' },
    date_string: { type: String, required: true },
    createdAt: { type: Date, default: Date.now }
});

// Tetap gunakan index unik harian agar nomor tidak kembar
QueueSchema.index({ queue_number: 1, date_string: 1 }, { unique: true });
// Index untuk admin mencari berdasarkan nama atau telepon jika perlu
